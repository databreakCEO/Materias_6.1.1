            ...
            try {
                ...
                do {
                    ...
                    if (linea != null) {
                        ...
                        for (String clave : claves) {
                            ...
                            for (int j = 0; j < lista.size(); j++) {
                                if (clave.equals(lista.get(j).codigoMat)) {
                                    horario.schedule.add(lista.get(j));
                                    band = true;
                                    break;
                                }
                            }
                        ...
                        filtrado.schedule.add(horario.schedule.get(0));

                        for (int i = 0; i < horario.schedule.size(); i++) {
                            band = true;
                            for (int j = 0; j < filtrado.schedule.size(); j++) {
                                band = true;
                                for (int k = 0; k < 5; k++) {
                                    int HIF = 0, HIS = 0, HFF, HFS;
                                    String[] finalesF = filtrado.schedule.get(j).horaFinales[k].split(":");
                                    String[] finalesS = horario.schedule.get(i).horaFinales[k].split(":");
                                    String[] iniciosF = filtrado.schedule.get(j).horaInicios[k].split(":");
                                    String[] iniciosS = horario.schedule.get(i).horaInicios[k].split(":");
...
//                        System.out.println(band);
                            if (band == true) {
                                filtrado.schedule.add(horario.schedule.get(i));
                            }
                        }
...
//                    horariosTerminados.add(horario);
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        ...
        try {
            ...
            modeloNombresMaterias.setRowCount(0);
            modeloNombresMaterias.setRowCount(horariosTerminados.get(r).schedule.size());
            for (int i = 7; i < 21; i++) {
                if (i == 12) {
                    jTable3.setValueAt((i) + " PM", i - 7, 0);
                } else if (i > 12) {
            ...
            for (int i = 0; i < horariosTerminados.get(r).schedule.size(); i++) {

                horariosTerminados.get(r).schedule.get(i).color = colores[i];
                jTable2.setValueAt(horariosTerminados.get(r).schedule.get(i).nombreMat, i, 0);
                colorNombres.addCelda(i, 0, colores[i]);
                colorNombres.addCelda(i, 1, colores[i]);

            }
            horariosTerminados.get(r).schedule.forEach((_item) -> {
