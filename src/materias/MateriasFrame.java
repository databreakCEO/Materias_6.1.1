...
//                        + horas[0] + " -" + horas[1] + " -" + horas[2] + " -" + horas[3] + " -" + horas[4] + " -" + line4);
...
//        }
    ...
    private void SiguienteHorarioActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SiguienteHorarioActionPerformed
        ...
        try {
            ...
            System.out.println(sc.get(numHorario).horaSalida + " - " + sc.get(numHorario).horaEntrada);
            jLabel36.setText("Numero de horas: " + (sc.get(numHorario).horaSalida - sc.get(numHorario).horaEntrada));
                ...
                while ("".equals(nombreA)) {
                    ...
                    if (nombreA != null) {
                        if (nombreA.length() > 0) {
                            ...
                            texto += nombreA + "->";
                            for (int i = 0; i < sc.get(numHorario).schedule.size(); i++) {
                                if (i < sc.get(numHorario).schedule.size() - 1) {
                                    texto += sc.get(numHorario).schedule.get(i).codigoMat + "-";
                                } else {
                                    texto += sc.get(numHorario).schedule.get(i).codigoMat;
                                }
...
//        OrdenarXHorasEscolares(sc, 0, sc.size() - 1, 0);
        ...
        try {
            ...
            for (int j = 0; j < lista.size(); j++) {
                if (materia.equals(lista.get(j).nombreMat.substring(0, lista.get(j).nombreMat.length())) && lista.get(j).aprobado) {
                    ...
                    horario.get(horario.size() - 1).schedule.add(lista.get(j));
                }
            }
        } catch (Exception e) {
    ...
    public void CrearCombinaciones(int ind, int n, Horario h) {
        for (int i = 0; i < horario.get(ind).schedule.size(); i++) {
            if (ind == 0) {
                h = new Horario();
                h.schedule.add(horario.get(ind).schedule.get(i));
                CrearCombinaciones(ind + 1, n, h);
            } else if (ind < n - 1) {
                Horario aux = new Horario();
                for (Materia schedule : h.schedule) {
                    aux.schedule.add(schedule);
                }
                if (aux.insertar(horario.get(ind).schedule.get(i))) {
    ...
    public void crearHorarios() {
        try {
            ...
            try {
                System.out.println(listaElegidos.size());
